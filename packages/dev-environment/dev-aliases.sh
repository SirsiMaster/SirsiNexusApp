...
# source ~/Development/dev-aliases.sh
...
# Function to quickly switch between projects with status
function proj() {
    if [ -z "$1" ]; then
    ...
    case "$1" in
        nexus)
            ~/Development/dev.sh go SirsiNexusApp
            ;;
        portal)
            ~/Development/dev.sh go SirsiMaster.github.io
            ;;
        assiduous)
            ~/Development/dev.sh go assiduous
            ;;
        pitch)
            ~/Development/dev.sh go sirsi-pitch-deck
            ;;
        *)
...
echo "✓ Development aliases and functions loaded"
echo "  Type 'dev help' for available commands"
echo "  Type 'proj' to navigate between projects"
