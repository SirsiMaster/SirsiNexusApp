...
#### gRPC Infrastructure Upgrade (Breaking Change — Internal Only)
- **Protobuf ES v2.11.0**: All TypeScript schemas regenerated via `buf.build/bufbuild/es`
- **Connect v2 Migration**: `createPromiseClient` → `createClient` across `sirsi-sign`
- **Plain Object Pattern**: Removed all `new Request()` constructors — v2 uses plain objects
- **V1 Cleanup**: Deleted 4 stale `proto/admin/v1` + `proto/contracts/v1` gen files (2,646 lines removed)
- **Orphan Cleanup**: Removed 12 unused `_connect.ts` files generated by intermediate v1 plugin
- **Firebase Pinning**: Locked `@firebase/app`, `@firebase/auth`, `@firebase/component`, `@firebase/logger`, `@firebase/util` to exact versions to resolve Rollup deep-import resolution failures
- **buf.gen.yaml**: Consolidated to single `buf.build/bufbuild/es` remote plugin (v2 generates both schemas AND service descriptors)

#### ADR-030 Phase 3: GitHub Automation & Catalog Sync
- **Go Backend — GitHub API**: `go-github/v60` integration for automatic tenant repo creation from `SirsiMaster/tenant-scaffold` template
- **StripeCatalogSync.tsx**: New reusable UCS component for Stripe product/price synchronization via gRPC
- **catalog.ts**: Canonical product/bundle definitions (Solo Plan, Business Plan, FinalWishes Core Bundle)
- **SystemSettings**: Integrated catalog sync UI into admin panel with real-time sync logs
- **AdminHeader**: Restored "Bipartite Authentication Active" gold badge
- **Provisioning Engine**: Backend tracks multi-step provisioning progress (Firebase → Stripe → GitHub → DNS)
- **Security**: GitHub integration gated by `SIRSI_GITHUB_PAT` env var — graceful degradation if absent

#### Build Verification
| Package | Status | Time |
|:---|:---|:---|
| `sirsi-sign` | ✅ PASS | 2.57s |
| `sirsi-portal-app` | ✅ PASS | 4.23s |
